...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        movie_csv = csv.reader(open(path, "r", encoding='utf-8'))
    ...
    def part_aii_2(self,connection, path):
        movie_cast_csv = csv.reader(open(path, "r", encoding='utf-8'))
    ...
    def part_b_1(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_b_1_sql = "CREATE INDEX movie_id\
                        ON movies (id)"
        ######################################################################
    ...
    def part_c(self,connection):
        ...
        part_c_sql = "SELECT printf(\"%.2f\", \
                (SELECT count(m1.id)\
                 FROM movies m1\
                 WHERE UPPER(m1.title) LIKE '%WAR%' AND m1.score > 50) * 100.0 / \
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT m1.title, printf(\"%.2f\",m1.score), count(m2.cast_id) cast_count\
                    FROM movies m1, movie_cast m2\
                    WHERE m1.id = m2.movie_id\
                    GROUP BY m1.title\
                    ORDER BY m1.score DESC, cast_count ASC, m1.title ASC\
                    LIMIT 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT m2.cast_id, m2.cast_name,printf(\"%.2f\",avg(m1.score)) AS avg_score\
                        FROM movies m1, movie_cast m2\
                       WHERE m1.id = m2.movie_id AND\
                             m1.score>=25\
                    GROUP BY m2.cast_id, m2.cast_name\
    ...
    def part_g(self,connection):
        ...
        part_g_sql = "CREATE VIEW good_collaboration AS\
                      SELECT m1.cast_id cast_member_id1,\
                             m2.cast_id cast_member_id2,\
                             COUNT(*) movie_count,\
                             AVG(m.score) average_movie_score\
                        FROM movies m, movie_cast m1, movie_cast m2\
                       WHERE m1.movie_id = m2.movie_id AND\
                             m1.cast_id < m2.cast_id AND\
                             m1.movie_id = m.id AND\
                             m2.movie_id = m.id\
                    GROUP BY m1.cast_id, m2.cast_id\
                      HAVING movie_count >=3 AND\
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT j.cast_id,\
                               mc.cast_name,\
                               printf('%.2f', AVG(j.average_movie_score)) collaboration_score\
                          FROM (SELECT cast_member_id1 as cast_id,\
                                ...
                                UNION ALL\
                                 SELECT cast_member_id2 as cast_id,\
                                        average_movie_score\
                                   FROM good_collaboration) j,\
                                        movie_cast mc\
                        WHERE mc.cast_id = j.cast_id\
                     GROUP BY j.cast_id, mc.cast_name\
    ...
    def part_h(self,connection,path):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        movie_overview_csv = csv.reader(open(path,"r", encoding = 'utf-8'))
    ...
    def part_hi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hi_sql = "SELECT count(overview)\
                         FROM movie_overview\
                        WHERE overview MATCH 'fight'"
               #WHERE CONTAINS ('overview','fight')
                ######################################################################
    ...
    def part_hii(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hii_sql = "SELECT count(overview)\
                          FROM movie_overview\
...
def part_g(connection):
    ############### EDIT SQL STATEMENT ###################################
    
    part_g_sql = "CREATE VIEW good_collaboration_4 AS\
                  SELECT m1.cast_id cast_member_id1,\
                         m2.cast_id cast_member_id2,\
                         COUNT(*) movie_count,\
                         AVG(m.score) average_movie_score\
                ...
                GROUP BY m1.cast_id, m2.cast_id\
                  ...
                  "
    ######################################################################
    execute_query(connection, part_g_sql)
    part_2_sql = "SELECT * FROM good_collaboration_4"
        ######################################################################
    cursor = connection.execute(part_2_sql)
    return cursor.fetchall()
