...
class HW2_sql():
    ...
    def part_f(self,connection):
        ...
        part_f_sql = "SELECT m2.cast_id, m2.cast_name,printf(\"%.2f\",avg(m1.score)) as avg_score\
                        FROM movies m1, movie_cast m2\
                        WHERE m1.id = m2.movie_id and m1.score>=25\
                        GROUP BY m2.cast_id, m2.cast_name\
                        HAVING count(movie_id) >= 3\
                        ORDER BY avg_score DESC, m2.cast_name ASC\
                        LIMIT 10"
        ######################################################################
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT j.cast_id, mc.cast_name, printf('%.2f', AVG(j.average_movie_score)) collaboration_score FROM \
                            (SELECT cast_member_id1 as cast_id, average_movie_score \
                             ...
                             FROM good_collaboration) j, movie_cast mc\
